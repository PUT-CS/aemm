...
export default function Column({ path, selectedChildPath, onItemClick }: ColumnProps) {
  ...
  return (
    <div className='border p-4 m-2 min-w-[200px]'>
      ...
      {isError && <div className='text-sm text-red-500'>Error loading content</div>}
      {data && data.children && (
          ...
          {data.children.map((item: any) => (
            <a
              ...
              className={`block p-2 rounded hover:bg-blue-100 cursor-pointer ${
                selectedChildPath === item.path ? 'bg-blue-200 font-semibold' : ''
              }`}
            >
